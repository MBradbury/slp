...
implementation 
{
	...
	int16_t calculate_sink_distance (int16_t msgSeqNo)
	{
		int16_t dis =  sink_source_distance - NON_SLEEP_CLOSER_TO_SOURCE - NON_SLEEP_CLOSER_TO_SINK;
		int16_t m = 2*(dis - 1);

		if (msgSeqNo % m <= dis && msgSeqNo % m != 0)
  		{
    		return msgSeqNo % m;
  		}
  		else
		...
		if (call NormalSeqNos.before_and_update(rcvd->source_id, rcvd->sequence_number))
		{
			...
			if (sleep_status == FALSE)
			{
				switch (nc)
				{
					case SleepNode:
					{
						if ((calculate_sink_distance(forwarding_message.sequence_number) == sink_distance || calculate_sink_distance(forwarding_message.sequence_number+1) == sink_distance)
							&& rnd <= SLEEP_PROBABILITY && sleep_timer > 0)
						{
							sleep_status = TRUE;
							sleep_timer --;
							//simdbg("stdout", "[Sequence:%d, distance: %d, ID: %d]: rnd = %d, P = %d. I am sleep.\n", forwarding_message.sequence_number, calculate_sink_distance(forwarding_message.sequence_number), TOS_NODE_ID, rnd, SLEEP_PROBABILITY);
						}
						else
						{
