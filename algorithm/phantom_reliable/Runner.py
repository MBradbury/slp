...
class RunSimulations(RunSimulationsCommon):
    ...
    def run(self, exe_path, distance, sizes, source_periods, walk_hop_lengths, walk_retries, configurations, attacker_models, repeats):
        ...
        argument_product = list(itertools.ifilter(
            lambda (size, source_period, retries, configuration, attacker_model, walk_length): walk_length in walk_hop_lengths[size],
            itertools.product(sizes, source_periods, walk_retries, configurations, attacker_models, set(itertools.chain(*walk_hop_lengths.values())))
        ))

        self.driver.total_job_size = len(argument_product)

        for (size, source_period, retries, configuration, attacker_model, walk_length) in argument_product:
            if not self._already_processed(repeats, size, source_period, walk_length, retries, configuration, attacker_model):
                ...
                options = 'algorithm.phantom ' + " ".join(opt_items)
