...
class Analyzer(AnalyzerCommon):
    ...
    def results_header():
        ...
        d['utility military']     = lambda x: str(Sigmoid.utility(x, [("Captured",   BattleSigmoid.cr),   # k value, x0 value, capture ratio(ranges from [0, 1])
                                                            ("ReceiveRatio",         BattleSigmoid.dr),   # k value, x0 value, receive ratio(ranges from [0, 1])
                                                            ("NormalLatency",        BattleSigmoid.lat),    # k value, x0 value, latency(scale is in seconds)
