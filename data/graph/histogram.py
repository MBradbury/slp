...
class Grapher(GrapherBase):
    ...
    def _write_plot_graph(self, dir_name, analysis):
        with open(os.path.join(dir_name, 'graph.gp'), 'w') as graph_p:
            ...
            graph_p.write('binwidth={}\n'.format(self.binwidth))
            graph_p.write('bin(x,width)=width*floor(x/width)\n')

            graph_p.write('plot "graph.dat" using (bin($1,binwidth)):(1.0) smooth freq with boxes\n')
